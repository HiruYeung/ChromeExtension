...
const likedLocation = {};
let currentPage = '';
let defaultPage = '長洲 Cheung Chau';
...
/* async function constructLocations() {
  const responses = await Promise.all([fetch('locationEn.txt'),fetch('locationZh.txt')])
  const [en, zh] = await Promise.all(responses.map(res => res.text()))
  const enList = en.split("\n")
  const zhList = zh.split("\n")

  locations = enList.map((postionStr, index) => {
    const [code, nameEn] = postionStr.split(": ");
    const nameZh = zhList[index].split(": ")[1];
...
function renderLocationOptions() {
    /* for (const location of locations) {
        ...
        const locationZhElement = document.createElement("p");
        const locationEnElement = document.createElement("p");

        locationZhElement.innerText = location.nameZh;
        locationEnElement.innerText = location.nameEn;
        locationEnElement.className = "enName";
        li.appendChild(locationZhElement);
...
/* function selectedLocation(location) {
    const locationZhElement = document.createElement("span");
    const locationEnElement = document.createElement("span");
    locationZhElement.innerText = location.nameZh + " ";
    locationEnElement.innerText = location.nameEn;
    locationEnElement.className = "enName";
    searchInput.replaceChildren(locationZhElement, locationEnElement);
    hideLcoationList();
} */
